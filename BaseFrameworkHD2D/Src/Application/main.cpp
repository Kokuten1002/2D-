...
void Application::PreDraw()
{
	...
	CamMat = Math::Matrix::CreateTranslation(0, 6.0f, -5.0f)* Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(angle++));//原点からの座標は絶対座標

	Math::Matrix CRotXMat = Math::Matrix::Identity;
	CRotXMat = Math::Matrix::CreateRotationX(DirectX::XMConvertToRadians(45));


	Math::Matrix CRotYMat = Math::Matrix::Identity;
	CRotYMat = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(angle++));



	Math::Matrix WorldMat = Math::Matrix::Identity;//ディグリー角度
	WorldMat = CRotXMat * CamMat * /*CRotYMat */ m_mat;
	//CamとCRはどこを原点としたいか？→m_mat
	//①原点の計算②親の行列を足すことでおやについてくようになる

	gameCamera.SetCameraMatrix(WorldMat);
...
{
	KdSquarePolygon* pCharacter = new KdSquarePolygon();
	pCharacter->SetMaterial("Asset/Textures/SampleChara.png");
	KdModelData* pTerrain = new KdModelData();
	pTerrain->Load("Asset/Data/Sample/Terrain/Terrain.gltf");
	// ===== ===== ===== ===== ===== ===== ===== ===== ===== ===== ===== =====
	...
	KdShaderManager::Instance().m_HD2DShader.BeginLit();
	{
		...
		//下の行列を覚える
		KdShaderManager::Instance().m_HD2DShader.DrawPolygon(*pCharacter, m_mat);
		KdShaderManager::Instance().m_HD2DShader.DrawModel(*pTerrain);
	...
	KdSafeDelete(pCharacter);
	KdSafeDelete(pTerrain);
}
